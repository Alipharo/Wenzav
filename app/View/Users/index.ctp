...
<?php foreach ($users as $user): ?>
	<tr>
		...
		<td><?php echo h($user['User']['gold']); ?>&nbsp;</td>
		<td class="actions">
                    <?php 
                        ...
                        if ($this->Session->check('Auth.User') && $this->Session->read('Auth.User.role') == "Admin") 
                        {
                            ...
                            echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $user['User']['id']), array('class' => 'btn btn-default btn-xs'), __('Are you sure you want to delete %s?', $user['User']['username']));                            
                        }
                    ?>		
		</td>
	</tr>
